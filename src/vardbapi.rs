...
enum EntryParseError {
    ...
    InvalidMTime(num::ParseIntError),
}

impl fmt::Display for EntryParseError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match *self {
            EntryParseError::MissingFields => write!(f, "Missing fields in CONTENTS line"),
            EntryParseError::InvalidDigest(ref e) => e.fmt(f),
            EntryParseError::InvalidMTime(ref e) => e.fmt(f),
        }
    }
}
...
impl error::Error for EntryParseError {
    fn source(&self) -> Option<&(dyn error::Error + 'static)> {
        match *self {
            EntryParseError::MissingFields => None,
            EntryParseError::InvalidDigest(ref e) => Some(e),
            EntryParseError::InvalidMTime(ref e) => Some(e),
        }
    }
}

impl From<DigestParseError> for EntryParseError {
    fn from(err: DigestParseError) -> EntryParseError {
        EntryParseError::InvalidDigest(err)
    }
}
...
fn parse_vardb_entry(line: &str) -> Result<FileMetadata, EntryParseError> {
    if line.len() < 5 {
    ...
    match prefix {
        "obj" => {
            ...
            let mtime = parts.next().ok_or_else(EntryParseError::MissingFields)?;
            let hash = parts.next().ok_or_else(EntryParseError::MissingFields)?;
            let path = path::PathBuf::from(parts.next().ok_or_else(EntryParseError::MissingFields)?);
            FileMetadata::REG(RegMetadata {
                // FIXME: Return the path
                hash: parse_hash(&hash)?,
                mtime: mtime.parse().map_err(|e| EntryParseError::InvalidMTime(e))?,
            })
        },
        "dir" => FileMetadata::DIR,
        "dev" => FileMetadata::DEV,
        "fif" => FileMetadata::FIFO,
        "sym" => {
            // sym <path> <mtime>
            let mut parts = line[4..].rsplitn(1, ' ');
            let mtime = parts.next().ok_or_else(EntryParseError::MissingFields)?;
            let path = path::PathBuf::from(parts.next().ok_or_else(EntryParseError::MissingFields)?);
            FileMetadata::LNK(LnkMetadata {
                mtime: mtime.parse().map_err(|e| EntryParseError::InvalidMTime(e))?,
            })
        },
    }
...
fn list_dirs(path: &path::Path) -> io::Result<impl Iterator<Item=path::PathBuf>> {
...
pub(in crate) fn parse_vdb(vdb_root: &path::Path, bases: &super::SearchPaths) -> io::Result<VarDB> {
    let mut expectations = VarDB::new();
